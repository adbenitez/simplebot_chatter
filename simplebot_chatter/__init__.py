...
def filter_messages(message: Message, bot: DeltaBot, replies: Replies) -> None:
    ...
    if reply_to_dash and message.text.startswith("#") and len(message.text) > 1:
        ...
        resp = CBOT.get_response(
            _rmprefix(_rmprefix(message.text, self_contact.addr), name)
            .strip(":,")
            .strip()
        )
    ...
    if resp:
        bot.logger.debug(
            "Confidence: %s | message: %s | reply: %s",
            resp.confidence,
            resp.in_response_to,
            resp.text,
        )
    ...
    if quote and quote.text and _getdefault(bot, "learn") == "1":
        CBOT.learn_response(Statement(text=message.text, in_response_to=quote.text))
